...
export const useVideoChatStore = defineStore('videoChatStore', {
  ...
  actions: {
    ...
    async recheckPermissions() {
      ...
      // 重新获取设备列表并更新 hasCamera/hasMic 状态
      ...
      message.success(`权限更新完成 - 摄像头:${this.hasCamera ? '✅' : '❌'} 麦克风:${this.hasMic ? '✅' : '❌'}`)
    },
    ...
    async accessDevice() {
      try {
        ...
        await navigator.mediaDevices
          ...
          .catch((error) => {
            console.log('❌ 麦克风权限被拒绝:', error.name, error.message)
            this.hasMicPermission = false
          })
        await navigator.mediaDevices
          .getUserMedia({
          ...
          .catch((error) => {
            console.log('❌ 摄像头权限被拒绝:', error.name, error.message)
            this.hasCameraPermission = false
          })
        const devices = await getDevices()
        this.devices = devices
    ...
    async fillStream(audioDeviceId: string, videoDeviceId: string) {
      ...
      const node = visionState.localVideoRef
        ...
        console.log('🔍 设备检测详情:', {
          总设备数: devices.length,
          音频设备: devices.filter(d => d.kind === 'audioinput').map(d => ({ label: d.label, id: d.deviceId })),
          视频设备: devices.filter(d => d.kind === 'videoinput').map(d => ({ label: d.label, id: d.deviceId })),
          权限状态: {
            hasMicPermission: this.hasMicPermission,
            hasCameraPermission: this.hasCameraPermission
          },
          ...
          用户设备: navigator.userAgent.includes('Mobile') ? '📱 移动设备' : '💻 桌面设备'
        })
      await getStream(
